...
The JWT Authentication policy validates JWT access tokens included in API requests. It verifies token signature, expiry, issuer, audience, scopes, and required claims using one or more JWKS (JSON Web Key Set) providers. It is typically applied to operations that require bearer token authentication before requests are forwarded upstream.
