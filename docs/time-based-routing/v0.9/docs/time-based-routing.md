...
When no schedule matches, the policy uses the optional `default` target. If no
default is configured, the original model and provider remain unchanged.
...
| `schedules[].target.model` | string | Yes | - | Model written to the upstream request. |
| `schedules[].target.provider` | string | No | primary provider | Additional-provider alias selected for this window. |
| `default.model` | string | No | original model | Model selected when no schedule matches. |
...
day and spans midnight. At all other times, the default target is used.
...
Because this configuration has no `default`, requests outside the configured
window retain their original model and provider.

## Routing metadata

When a schedule or the default target is selected, the policy publishes the
...
| `time_based_routing.selected_route` | Configured schedule name, generated `schedule-N` name, or `default`. |
